...
describe('addTwoPromises', () => {
    test('resolves with the sum of two resolved promises (2 + 2)', async () => {
        const result = await addTwoPromises(Promise.resolve(2), Promise.resolve(2));
        expect(result).toBe(4);
    });

    test('resolves with the sum when promises resolve after different timeouts (2 + 5)', async () => {
    ...
    test('resolves with the sum when one promise resolves to a negative number (10 + -12)', async () => {
    ...
    test('resolves with the sum when both promises resolve to zero', async () => {
        const result = await addTwoPromises(Promise.resolve(0), Promise.resolve(0));
        expect(result).toBe(0);
    });

    test('resolves with the sum when both promises resolve to negative numbers', async () => {
